...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    type VARCHAR(50), -- payment, refund, payout
...
CREATE TABLE IF NOT EXISTS traffic_sources (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    source VARCHAR(100) NOT NULL, -- direct, search, social, referral
...
-- Messages Table (for Message Center)
...
-- Orders: Open access for now (adjust based on your needs)
CREATE POLICY "Enable read access for orders" ON orders
    FOR SELECT USING (true);

CREATE POLICY "Enable insert access for orders" ON orders
